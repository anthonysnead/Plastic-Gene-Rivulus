...
Sample.Data <- Sample.Data[ order(row.names(Sample.Data)), ] %>%
...
# Here I create and interactive table with the deferentially expressed genes. the comparison column is the test that was performed. 
...
Here we are going to cluster using hierarchical clustering methods.

#### Data Preparation for Clustering

We first model the mean-variance relationship and normalize the data for clustering.

```{r - Clustering Data Preparation}
V.DGE.Data <- limma::voom(DGE.Data, Design, plot = TRUE)
# Here we mode the mean-variance relationship and get observation-level weights to run clustering

Diffgenes <- V.DGE.Data$E %>%
  dplyr::as_tibble(rownames = "Gene") %>%
  dplyr::filter(Gene %in% rownames(F.tests$PreCold.PreWarm$Significant.Table$table)|Gene %in% rownames(F.tests$PreCold.PostCold$Significant.Table$table)|Gene %in% rownames(F.tests$PostCold.PostWarm$Significant.Table$table)) %>%
  tibble::column_to_rownames(., var = "Gene") %>%
  as.matrix(.)
# Here we filter out genes that are not deferentially expressed.
...
Here we only use deferentially expressed gene to cluster. We keep each sample separate. 

```{r - Clustering By Sample}
clustRow <- hclust(as.dist(1-cor(t(Diffgenes), method = "pearson")), method = "complete")
# Here we cluster by Row.

clustColumns <- hclust(as.dist(1-cor((Diffgenes), method = "spearman")), method = "complete")
# Here we cluster by columns or samples.

module.assign <- cutree(clustRow, k = 4)
# We assign genes to clusters

module.color <- module.assign %>%
  tibble::as_tibble(.) %>%
  ...
  dplyr::rename(Modules = value) 

heat.map <- heatmaply::heatmaply(Diffgenes,
                              Rowv = as.dendrogram(clustRow),
                              Colv = as.dendrogram(clustColumns),
                              RowSideColors = module.color,
                              #col_side_colors = NULL,
                              scale = c("row"),
                              colors = gplots::bluered(75),
                              row_dend_left = TRUE,
                              key.title = "Scaled By Row",
                              trace = 'none',
                              main = c("Differentially Expressed Genes Heatmap"),
                              xlab = c("Samples"))
# Here we make a heatmap with the deferentially expressed Genes scaled by the row to emphasize sample differences.
...
Here I group samples by the treatment then cluster. We group all warm treatments together because they had no differentially expressed genes.

```{r - Cluster By Group}

Diffgenes.Group <- Diffgenes
# We create a copy of Diffgenes to group by for a group level heatmap.

colnames(Diffgenes.Group) <- c("PostCold", "PostCold", "PostCold", "Warm", "Warm", "Warm", "PreCold","PreCold","PreCold","Warm","Warm","Warm")
# We rename the columns for grouping. We do not separate the Pre versus Post Warm because they have no deferentially expressed genes between them.

Avg.V.DGE <- limma::avearrays(Diffgenes.Group)
# We get group level averages.

Avg.clustRow <- hclust(as.dist(1-cor(t(Avg.V.DGE), method = "pearson")), method = "complete")
# Here we cluster by Row.

Avg.clustColumns <- hclust(as.dist(1-cor((Avg.V.DGE), method = "spearman")), method = "complete")
# Here we cluster by columns or samples.

Avg.module.assign <- cutree(Avg.clustRow, k = 4)
# We assign genes to clusters

Avg.module.color <- Avg.module.assign %>%
  tibble::as_tibble(.) %>%
  dplyr::mutate(value = case_when(
    value == 1 ~ "blue",
    value == 2 ~ "purple",
...
Avg.heat.map <- heatmaply::heatmaply(Avg.V.DGE,
                              Rowv = as.dendrogram(Avg.clustRow),
                              Colv = as.dendrogram(Avg.clustColumns),
                              row_side_colors = Avg.module.color,
                              #col_side_colors = NULL,
                              scale = "row",
                              colors = gplots::bluered(75),
                              row_dend_left = TRUE,
...
# Here we make a heatmap with the deferentially expressed Genes scaled by the row to emphasize group differences.
...
Fishers.gost.table <- as_tibble(Fishers.gost.res$result) %>% 
  mutate(Direction = case_when(stringr::str_detect(query,"Up") ~ "Up",  stringr::str_detect(query, "Down") ~ "Down")) %>% dplyr::mutate(query = case_when( stringr::str_detect(query, "PrC_PrW") ~ "PrC_PrW", stringr::str_detect(query, "PrC_PoC") ~ "PrC_PoC", stringr::str_detect(query, "PoC_PoW") ~ "PoC_PoW")) %>% rename(Comparision = query)
# Here we convert the results into a tibble and swap query for actual comparison names.

Fishers.GO.Counts <- Fishers.gost.table %>% group_by(Comparision, Direction, source) %>% tally(.)
...
Here we complete a ranked enrichment analysis for GO terms. We create interactive tables and plots to help with data exploration. The queries are in the same order as the data list. Therefore, query 1 represents PreCold versus PreWarm up regulated genes.

```{r - Go Ranked Test}
Ranked.gost.res <- gost(GO.input.list, organism = "kmarmoratus", correction_method = "fdr", exclude_iea = FALSE, sources = c("GO:BP", "GO:CC", "GO:MF"), ordered_query = TRUE, domain_scope = "annotated")
# We run gost on all the GO libraries. We use a list of the data which is differentiated by the query column. The ordered_query = TRUE means that we run a ranked test.

gostplot(Ranked.gost.res, interactive = TRUE, capped = TRUE) 
# Here we make an interactive Manhattan plot.

Ranked.gost.table <- as_tibble(Ranked.gost.res$result) %>%  mutate(Direction = case_when(stringr::str_detect(query,"Up") ~ "Up",  stringr::str_detect(query, "Down") ~ "Down")) %>% dplyr::mutate(query = case_when( stringr::str_detect(query, "PrC_PrW") ~ "PrC_PrW", stringr::str_detect(query, "PrC_PoC") ~ "PrC_PoC", stringr::str_detect(query, "PoC_PoW") ~ "PoC_PoW")) %>% rename(Comparision = query)
# Here we convert the results into a tibble and swap query for actual comparison names.

Ranked.GO.Counts <- Ranked.gost.table %>% group_by(Comparision, Direction, source) %>% tally(.)
# We count the various GO terms in each category.

DT::datatable(Ranked.gost.table,
                              extensions = c('KeyTable', 'FixedHeader'),
                              filter = 'top',
                              options = list(keys = TRUE,
                                             searchHighlight = TRUE,
